...
from .tasks import bridge_literature_task, log_audit_event, perform_reflection_task
...
AGENT_ID = "literature_bridger_agent_001"

app = FastAPI(
    title=f"AlzNexus Specialized Sub-Agent Service: {AGENT_ID}",
    description="A specialized sub-agent that scans scientific literature, extracts key information, and synthesizes connections.",
    version="1.0.0",
)

LITERATURE_BRIDGER_AGENT_API_KEY = os.getenv("LITERATURE_BRIDGER_AGENT_API_KEY")
LITERATURE_BRIDGER_REDIS_URL = os.getenv("LITERATURE_BRIDGER_REDIS_URL", "redis://localhost:6379")
...
if not LITERATURE_BRIDGER_AGENT_API_KEY:
    raise ValueError("LITERATURE_BRIDGER_AGENT_API_KEY environment variable not set.")
...
async def get_api_key(api_key: str = Security(api_key_header)):
    if api_key == LITERATURE_BRIDGER_AGENT_API_KEY:
        return api_key
...
async def register_self_with_registry():
    ...
    registration_payload = schemas.AgentRegister(
        agent_id=AGENT_ID,
        capabilities={
            "description": "Scans scientific literature, extracts key information, and synthesizes connections between disparate research areas.",
            "tools": ["ADWorkbenchQueryTool", "LiteratureAnalysisTool", "KnowledgeGraphSynthesisTool"],
            "domain": "Literature Bridging"
        },
...
async def startup_event():
    ...
    redis_connection = redis.from_url(LITERATURE_BRIDGER_REDIS_URL, encoding="utf-8", decode_responses=True)
...
async def execute_task(
    ...
    """Instructs the Literature Bridger agent to execute a literature bridging task asynchronously."""
    ...
    bridge_literature_task.delay(db_agent_task.id)
...
async def health_check():
    ...
    return {"status": "healthy", "service": "literature_bridger_agent"}
