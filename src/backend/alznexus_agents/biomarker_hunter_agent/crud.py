...
def get_recent_agent_tasks(db: Session, agent_id: str, days_ago: int = 7):
    """Retrieves recent tasks for a given agent within a specified number of days."""
    time_threshold = datetime.utcnow() - timedelta(days=days_ago)
    return db.query(models.AgentTask).filter(
        models.AgentTask.agent_id == agent_id,
        models.AgentTask.created_at >= time_threshold
