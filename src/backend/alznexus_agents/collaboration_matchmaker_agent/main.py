...
from .tasks import match_collaboration_task, log_audit_event, perform_reflection_task
...
AGENT_ID = "collaboration_matchmaker_agent_001"

app = FastAPI(
    title=f"AlzNexus Specialized Sub-Agent Service: {AGENT_ID}",
    description="A specialized sub-agent that identifies complex research problems and suggests optimal multi-agent teams or external human experts for collaboration.",
    version="1.0.0",
)

COLLABORATION_MATCHMAKER_AGENT_API_KEY = os.getenv("COLLABORATION_MATCHMAKER_AGENT_API_KEY")
COLLABORATION_MATCHMAKER_REDIS_URL = os.getenv("COLLABORATION_MATCHMAKER_REDIS_URL", "redis://localhost:6379")
...
if not COLLABORATION_MATCHMAKER_AGENT_API_KEY:
    raise ValueError("COLLABORATION_MATCHMAKER_AGENT_API_KEY environment variable not set.")
...
async def get_api_key(api_key: str = Security(api_key_header)):
    if api_key == COLLABORATION_MATCHMAKER_AGENT_API_KEY:
        return api_key
...
async def register_self_with_registry():
    ...
    registration_payload = schemas.AgentRegister(
        agent_id=AGENT_ID,
        capabilities={
            "description": "Identifies complex research problems and suggests optimal multi-agent teams or external human experts for collaboration.",
            "tools": ["ADWorkbenchQueryTool", "AgentRegistryQueryTool", "ExpertMatchingTool"],
            "domain": "Collaboration Matchmaking"
        },
        api_endpoint=AGENT_EXTERNAL_API_ENDPOINT
    ).model_dump_json()
...
async def startup_event():
    ...
    redis_connection = redis.from_url(COLLABORATION_MATCHMAKER_REDIS_URL, encoding="utf-8", decode_responses=True)
    await FastAPILimiter.init(redis_connection)
    print("FastAPI-Limiter initialized for Agent Service.")
    await register_self_with_registry()
...
async def execute_task(
    ...
    """Instructs the Collaboration Matchmaker agent to execute a collaboration matchmaking task asynchronously."""
    ...
    match_collaboration_task.delay(db_agent_task.id)
...
async def health_check():
    ...
    return {"status": "healthy", "service": "collaboration_matchmaker_agent"}
