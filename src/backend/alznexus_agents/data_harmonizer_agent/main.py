...
from .tasks import harmonize_data_task, log_audit_event, perform_reflection_task
...
AGENT_ID = "data_harmonizer_agent_001"

app = FastAPI(
    title=f"AlzNexus Specialized Sub-Agent Service: {AGENT_ID}",
    description="A specialized sub-agent for data harmonization and schema alignment.",
    version="1.0.0",
)

HARMONIZER_AGENT_API_KEY = os.getenv("HARMONIZER_AGENT_API_KEY")
HARMONIZER_REDIS_URL = os.getenv("HARMONIZER_REDIS_URL", "redis://localhost:6379")
...
if not HARMONIZER_AGENT_API_KEY:
    raise ValueError("HARMONIZER_AGENT_API_KEY environment variable not set.")
...
async def get_api_key(api_key: str = Security(api_key_header)):
    if api_key == HARMONIZER_AGENT_API_KEY:
        return api_key
...
async def register_self_with_registry():
    ...
    registration_payload = schemas.AgentRegister(
        agent_id=AGENT_ID,
        capabilities={
            "description": "Aligns schemas from disparate studies, ensuring data consistency and interoperability.",
            "tools": ["ADWorkbenchDataScanTool", "SchemaAlignmentTool"],
...
async def startup_event():
    ...
    redis_connection = redis.from_url(HARMONIZER_REDIS_URL, encoding="utf-8", decode_responses=True)
...
async def execute_task(
    ...
    """STORY-304: Instructs the Data Harmonizer sub-agent to execute a data harmonization task asynchronously."""
    ...
    harmonize_data_task.delay(db_agent_task.id)
...
async def health_check():
    ...
    return {"status": "healthy", "service": "data_harmonizer_agent"}
