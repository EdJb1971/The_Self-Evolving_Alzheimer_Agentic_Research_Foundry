...
class AgentTask(Base):
    __tablename__ = "trial_optimizer_agent_tasks"

    id = Column(Integer, primary_key=True, index=True)
    agent_id = Column(String, nullable=False)
    orchestrator_task_id = Column(Integer, nullable=True) # Link back to orchestrator task if applicable
...
class AgentState(Base):
    __tablename__ = "trial_optimizer_agent_states"

    id = Column(Integer, primary_key=True, index=True)
    agent_id = Column(String, unique=True, nullable=False)
    current_goal = Column(Text, nullable=True)
    current_task_id = Column(Integer, nullable=True) # Current active task
    last_reflection_at = Column(DateTime(timezone=True), nullable=True)
    metadata_json = Column(JSON, nullable=True) # General agent state/config
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
