...
from .tasks import optimize_trial_task, log_audit_event, perform_reflection_task
...
app = FastAPI(
    ...
    description="A specialized sub-agent for clinical trial design and optimization.",
    version="1.0.0",
)

TRIAL_OPTIMIZER_AGENT_API_KEY = os.getenv("TRIAL_OPTIMIZER_AGENT_API_KEY")
TRIAL_OPTIMIZER_REDIS_URL = os.getenv("TRIAL_OPTIMIZER_REDIS_URL", "redis://localhost:6379")
...
if not TRIAL_OPTIMIZER_AGENT_API_KEY:
    raise ValueError("TRIAL_OPTIMIZER_AGENT_API_KEY environment variable not set.")
...
async def get_api_key(api_key: str = Security(api_key_header)):
    if api_key == TRIAL_OPTIMIZER_AGENT_API_KEY:
        return api_key
...
async def register_self_with_registry():
    ...
    registration_payload = schemas.AgentRegister(
        agent_id=AGENT_ID,
        capabilities={
            "description": "Designs and optimizes clinical trial protocols, including patient selection, endpoints, and dosage.",
            "tools": ["ADWorkbenchQueryTool", "TrialDesignOptimizationTool"],
            "domain": "Clinical Trial Optimization"
        },
...
async def startup_event():
    ...
    redis_connection = redis.from_url(TRIAL_OPTIMIZER_REDIS_URL, encoding="utf-8", decode_responses=True)
...
async def execute_task(
    ...
    """STORY-302: Instructs the Trial Optimizer sub-agent to execute a trial optimization task asynchronously."""
    ...
    optimize_trial_task.delay(db_agent_task.id)
...
async def health_check():
    ...
    return {"status": "healthy", "service": "trial_optimizer_agent"}
